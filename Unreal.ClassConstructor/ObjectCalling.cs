...
namespace Unreal.ClassConstructor
{
    internal partial class ObjectMethods : ModuleBase<ClassConstructorContext>, IObjectMethods
    {
        ...
        {
            if (!GetFunction(obj, funcName, out UFunction* targetFunc)) return default;
            var pParams = ProcessEventAllocParameterStorage(targetFunc);
            var processEventWrapper = _context._utils.MakeWrapper<UObject_ProcessEvent>(*(nint*)(obj->_vtable + 0x220));
        ...
        {
            UWorld* currWorld = GetCurrentWorld();
        ...
        public unsafe UWorld* GetCurrentWorld()
        {
            var worldList = &(*__classHooks.GEngine)->WorldList;
            for (int i = 0; i < worldList->arr_num; i++)
            {
                var currWorldCtx = worldList->Get<FWorldContext>(i);
                if (currWorldCtx != null && currWorldCtx->ThisCurrentWorld != null)
                    return currWorldCtx->ThisCurrentWorld;
            }
            return null;
        }
